...
static void *
conn_init(void *opaque, unsigned char *local, int locallen,
	...
	if (locallen == 4) {
		logger_log(conn->raop->logger, LOGGER_INFO, "Local: %d.%d.%d.%d",
	...
	if (remotelen == 4) {
		logger_log(conn->raop->logger, LOGGER_INFO, "Remote: %d.%d.%d.%d",
		...
		logger_log(conn->raop->logger, LOGGER_INFO,
				...
				remote[10], remote[11], remote[12], remote[13], remote[14],
				remote[15]);
	}
...
static void conn_request(void *ptr, http_request_t *request,
	...
	if (strcmp(method, "OPTIONS") != 0 && strlen(raop->password)) {
		...
		if (!digest_is_valid(realm, raop->password, conn->nonce, method,
				http_request_get_url(request), authorization)) {
			char *authstr;
			int authstrlen;

			/* Allocate the authenticate string */
			authstrlen = sizeof("Digest realm=\"\", nonce=\"\"") + sizeof(realm)
					+ sizeof(conn->nonce) + 1;
	...
	http_response_add_header(res, "Apple-Jack-Status",
	...
	if (require_auth) {
		...
		http_response_add_header(res, "Public",
		...
		if (data) {
			...
			logger_log(conn->raop->logger, LOGGER_DEBUG, "connection: %s",
					remotestr);
			logger_log(conn->raop->logger, LOGGER_DEBUG, "rtpmap: %s",
					rtpmapstr);
			...
			conn->raop_rtp = raop_rtp_init(raop->logger, &raop->callbacks,
		...
		if (conn->raop_rtp) {
			raop_rtp_start(conn->raop_rtp, use_udp, remote_cport, remote_tport,
					&cport, &tport, &dport);
		} else {
			logger_log(conn->raop->logger, LOGGER_ERR,
...
raop_init(int max_clients, raop_callbacks_t *callbacks, const char *pemkey,
	...
	if (!callbacks->audio_init || !callbacks->audio_process
...
raop_t *
raop_init_from_keyfile(int max_clients, raop_callbacks_t *callbacks,
		const char *keyfile, int *error) {
	raop_t *raop;
	char *pemstr;
...
int raop_start(raop_t *raop, unsigned short *port, const char *hwaddr,
